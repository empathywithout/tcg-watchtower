...
(async function() {
  ...
  function closeAllMenus() {
    ...
    const rbV = document.getElementById('riftbound-sets-view'); if (rbV) rbV.classList.remove('open');
  ...
  const setsGridContainerRb = document.getElementById('sets-grid-container-rb');
  const filterTabsRb = document.querySelectorAll('#sets-filter-tabs-rb .filter-tab');
  let currentFilterRb = 'all';
  let rbSetsLoaded = false;
  if (riftboundMenuItem) riftboundMenuItem.addEventListener('click', () => {
    hamburgerMenu.classList.remove('open');
    const rbV = document.getElementById('riftbound-sets-view');
    if (rbV) { rbV.classList.add('open'); renderRiftboundSets(); }
  });
  if (backToMenuRb) backToMenuRb.addEventListener('click', backToMainMenu);
  filterTabsRb.forEach(tab => {
    tab.addEventListener('click', () => {
      filterTabsRb.forEach(t => t.classList.remove('active'));
      tab.classList.add('active');
      currentFilterRb = tab.dataset.filter;
      renderRiftboundSets();
    });
  });
  async function renderRiftboundSets() {
    if (!setsGridContainerRb) return;
    if (!rbSetsLoaded) {
      try {
        ...
        window._rbSets = sets;
        rbSetsLoaded = true;
      } catch(e) { setsGridContainerRb.innerHTML = '<div style="color:var(--muted);text-align:center;padding:40px">Could not load sets</div>'; return; }
    }
    const sets = window._rbSets || [];
    const filtered = currentFilterRb === 'live' ? sets.filter(s => s.live) : sets;
    const r2 = 'https://pub-20ee170c554940ac8bfcce8af2da57a8.r2.dev';
    let html = '<div class="sets-grid">';
    filtered.forEach(set => {
      const disabled = !set.live;
      const logoUrl = \`\${r2}/logos/riftbound/\${set.setId}.webp\`;
      html += \`<a href="\${disabled ? 'javascript:void(0)' : '/riftbound/sets/' + set.slug + '/cards'}" class="set-card\${disabled ? ' disabled' : ''}">
        <div class="set-card-image"><img src="\${logoUrl}" alt="\${set.name}" style="width:85%;max-width:130px;height:auto;object-fit:contain;" onerror="this.style.display='none'"></div>
        <div class="set-card-content">
          <div class="set-card-name">\${set.name}</div>
          <div class="set-card-info">\${set.short} • Riftbound TCG</div>
          \${disabled ? '<span class="set-card-soon">Coming Soon</span>' : ''}
        </div></a>\`;
    });
    html += '</div>';
    setsGridContainerRb.innerHTML = html;
  }
...
for (const file of files) {
  ...
  // 3. Replace initNav JS block — replace if One Piece or Riftbound support missing
  const before3 = content;
  if (content.includes('function initNav()') && (!content.includes('onepieceMenuItem') || !content.includes('renderRiftboundSets'))) {
    content = content.replace(
