...
<script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','G-E0S4363S5Y',{set_id:'${SET_ID}',series:'One Piece',page_type:'set_list'});</script>
...
.section-nav-btn{flex:1;padding:10px 4px;font-size:.65rem;font-weight:700;text-transform:uppercase;letter-spacing:0;color:var(--muted);background:none;border:none;border-bottom:2px solid transparent;cursor:pointer;transition:color .2s,border-color .2s;white-space:nowrap;font-family:inherit;text-align:center}
...
.section-nav-sets-btn{flex:1;padding:10px 4px;font-size:.65rem;font-weight:700;text-transform:uppercase;letter-spacing:0;color:var(--muted);background:none;border:none;border-bottom:2px solid transparent;cursor:pointer;transition:color .2s;white-space:nowrap;font-family:inherit;text-align:center;width:100%}
...
.chase-card{flex:0 0 200px;scroll-snap-align:start;background:rgba(30,41,59,.7);backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,.08);border-radius:16px;overflow:hidden;transition:transform .3s,box-shadow .3s;cursor:pointer;display:flex;flex-direction:column;height:520px}
...
.modal-rarity-badge{display:inline-flex;align-items:center;padding:3px 10px;border-radius:999px;font-size:.7rem;font-weight:700;text-transform:uppercase;letter-spacing:.05em;background:rgba(245,158,11,.15);border:1px solid rgba(245,158,11,.3);color:#fbbf24;margin-bottom:12px}
.modal-price-box{background:rgba(15,23,42,.7);border:1px solid rgba(255,255,255,.08);border-radius:10px;padding:12px;text-align:center;margin-bottom:12px}
.modal-price-label{font-size:.65rem;color:var(--muted);text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px}
.modal-price-value{font-size:1.4rem;font-weight:700;color:var(--amber);font-family:'Space Mono',monospace}
.modal-links{margin-top:0;display:flex;flex-direction:column;gap:10px}
...
<nav class="section-nav" id="section-nav">
  <div class="section-nav-inner">
    ...
    <button class="section-nav-btn" data-target="section-products" data-url="/one-piece/sets/${SET_URL_SLUG}/sealed-product"><span class="nav-full">Sealed Products</span><span class="nav-short">Sealed</span></button>
    <div class="section-nav-sets">
      <button class="section-nav-sets-btn" id="nav-sets-btn"><span class="nav-full">All Sets ▾</span><span class="nav-short">All Sets ▾</span></button>
    </div>
  </div>
...
<section class="section">
  <div class="container" style="text-align:center">
    ...
    <p style="color:var(--muted);max-width:560px;margin:0 auto 36px">
      Join 1,800+ collectors on Discord and get instant notifications when ${SET_SHORT_NAME} Booster Boxes, ETBs, and sealed product goes live at major retailers.
    </p>
    <div style="display:flex;gap:16px;justify-content:center;flex-wrap:wrap">
      <a href="https://discord.gg/cZxJV9YRyb" class="btn-primary">
      ...
      <a href="/" class="btn-secondary">Browse All Alerts</a>
    </div>
  </div>
</section>
...
function cardPageUrl(localId, rawName) {
  return \`/one-piece/sets/\${SET_URL_SLUG}/cards/\${toSlug(rawName)}-\${toSlug(localId)}\`;
}
...
function renderChaseCards(cards) {
  currentChaseList = cards
    ...
    .map(c=>({
      ...
      img:/^[A-Z]{2,}\d+-/.test(c.localId) ? cardImg(c.localId) : (c.image||cardImg(c.localId)),
    }));
  renderChaseHTML();
}
  ...
  document.getElementById('chase-grid').innerHTML = sorted.map(c=>{
    ...
    return \`<div class="chase-card" data-localid="\${c.id}" data-rawname="\${(c.rawName||c.name).replace(/"/g,'&quot;')}" data-name="\${c.name.replace(/"/g,'&quot;')}" data-rarity="\${c.rarity}" data-img="\${c.img}" onclick="handleChaseClick(this)">
      ...
      <div class="chase-card-info">
        ...
        <div class="chase-card-number">\${c.displayId}</div>
...
function renderCards(reset) {
  ...
  slice.forEach(card => {
    ...
    const imgUrl = /^[A-Z]{2,}\d+-/.test(card.localId) ? cardImg(card.localId) : (card.image||cardImg(card.localId));
    ...
    el.addEventListener('click',()=>{ const c2=getCached(card.localId,card.name); openModal(card.localId,card.name,card.rarity||'',imgUrl,c2?.url||null,card.name); });
    grid.appendChild(el);
    rendered++;
  });
...
function handleChaseClick(el) {
  ...
  openModal(localId,name,rarity,imgUrl,cached?.url||null,rawName);
}

function openModal(localId,name,rarity,imgUrl,directUrl,rawName) {
  rawName = rawName || name;
  if(!imgUrl)imgUrl=cardImg(localId);
  const cached=getCached(localId,rawName);
  const dispNum=displayNumber(localId);
  const tcgpUrl=directUrl||cached?.url||tcgpLink(rawName,dispNum);
  document.getElementById('modal-inner').innerHTML=\`
    ...
    <div>
      ...
      \${rarity?\`<div class="modal-rarity-badge">\${rarity}</div>\`:''}
      <div class="modal-price-box">
        <div class="modal-price-label">Market Price</div>
        <div class="modal-price-value">\${cached?.price ? '$' + cached.price.toFixed(2) : '—'}</div>
      </div>
      <div class="modal-links">
        <a class="modal-buy-link pl-amazon" href="\${amazonLink(rawName+' '+dispNum+' '+SET_FULL_NAME+' One Piece Card')}" target="_blank" rel="noopener"><span>Amazon</span><span>→</span></a>
        <a class="modal-buy-link pl-ebay" href="\${ebayLink(rawName+' '+dispNum+' '+SET_FULL_NAME+' One Piece Card')}" target="_blank" rel="noopener"><span>eBay</span><span>→</span></a>
        <a class="modal-buy-link pl-tcgp" href="\${tcgpUrl}" target="_blank" rel="noopener"><span>TCGplayer</span><span>→</span></a>
