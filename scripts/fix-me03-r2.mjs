...
async function main() {
  ...
  while (true) {
    const res = await fetch(
      `https://api.scrydex.com/pokemon/v1/expansions/${JP_ID}/cards?select=id,name,slug,rarity,images&pageSize=100&page=${page}`,
      { headers: HEADERS }
    );
  ...
  const cards = all.map(c => {
    ...
    const slug = c.slug || '';
    const name = slug ? slugToName(slug) : c.name || '';
