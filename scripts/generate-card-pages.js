...
<script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','G-E0S4363S5Y',${dimsJson});</script>
...
function generateCardPage(card, allCards) {
  ...
  const cardTotal   = card.denominator || metadata.cardCount?.official || '';
  const cardNum     = cardTotal ? `${card.localId}/${cardTotal}` : card.localId;
  const title       = `${card.name} ${cardNum} — ${SET_FULL_NAME} | Price & Card Info | TCG Watchtower`;
...
@media(max-width:768px){.card-layout{grid-template-columns:1fr}.card-image-wrap img{max-height:50vh;width:auto;margin:0 auto;display:block}}
.card-image-wrap{position:relative}
@media(min-width:769px){.card-image-wrap{position:sticky;top:72px}}
.card-image-wrap img{width:100%;height:auto;border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,0.5);transition:transform 0.3s}
...
<div class="container">
  <div class="card-layout">
    <div class="card-image-wrap">
      <img src="${img}" alt="${card.name} #${card.localId} ${SET_FULL_NAME}" width="400"
...
.buy-link{flex:1;padding:3px 3px;border-radius:6px;font-size:0.6rem;font-weight:700;white-space:nowrap;overflow:hidden;text-decoration:none;transition:all 0.2s;display:inline-flex;align-items:center;justify-content:center;gap:4px;text-align:center;min-height:44px}
  ...
  return cardList.map(c => {
    ...
    <div class="card-item">
      ...
      <div class="card-info">
        ...
        <div class="buy-links">
          <a class="buy-link buy-amazon" href="https://www.amazon.com/s?k=${encodeURIComponent(c.name + ' ' + SET_FULL_NAME + ' Pokemon Card')}&linkCode=ll2&tag=cehutto01-20&language=en_US" target="_blank" rel="noopener">Amazon</a>
