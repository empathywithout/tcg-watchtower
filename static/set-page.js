...
function renderCards(reset) {
  ...
  slice.forEach(card => {
    ...
    const fallbackUrl = SET_PHASE === 'jp' ? (card.image || null) : (card.fallbackImage || null);
  ...
  inner.innerHTML = `
    ...
    <div>
      ...
      <div class="modal-price-box">
        ...
        <div class="modal-price-value">${priceCache[localId]?.price ? '$' + priceCache[localId].price.toFixed(2) : priceCache[localId.padStart(3,'0')]?.price ? '$' + priceCache[localId.padStart(3,'0')].price.toFixed(2) : '—'}</div>
      </div>
...
(async function() {
  ...
  function toggleHamburgerMenu() {
    ...
    if (isOpen) { closeAllMenus(); }
      ...
      hamburgerMenu.style.top = (rect.bottom + 8) + 'px';
      hamburgerMenu.style.right = (window.innerWidth - rect.right) + 'px';
