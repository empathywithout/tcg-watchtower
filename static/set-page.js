    ...
    currentChaseList = cards
      ...
      .map(c => {
        ...
        return {
          ...
          img: SET_PHASE === 'jp' ? cardImg(SET_ID, c.localId) : (c.image || cardImg(SET_ID, c.localId)),
        };
      });
  } else {
...
function renderCards(reset) {
  ...
  slice.forEach(card => {
    const imgUrl = SET_PHASE === 'jp'
      ? (cardImg(SET_ID, card.localId) || card.image)
      : (card.image || cardImg(SET_ID, card.localId));
