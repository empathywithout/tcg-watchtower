...
// Uses TCGCSV (TCGplayer mirror) for card names + rarities in 2 requests,
...
// Maps set ID to TCGplayer group ID (for TCGCSV)
const SET_TO_GROUP = {
  'sv01': '22788',
  'sv02': '22873',
  'sv03': '23100',
...
export default async function handler(req, res) {
  ...
  if (!setId || !/^[a-z0-9]+(\.[0-9])?$/.test(setId)) {
  ...
  try {
    ...
    // Strategy 1: TCGCSV — gets all cards with rarity in 2 parallel requests
    if (groupId) {
      try {
        ...
        if (productsRes.ok) {
          ...
          // Filter to cards only (have a card number, not sealed product)
          ...
          if (cardRows.length > 0) {
            ...
            cards = cardRows.map(p => {
              // Preserve leading zeros for localId (e.g. "001")
              const localId = p.number;
              return {
                localId,
                name: p.name,
                image: `${r2Base}/cards/${setId}/${localId}.webp`,
    ...
    // Strategy 2: TCGdex fallback (set endpoint + per-card for rarity)
    if (!cards || cards.length === 0) {
      ...
      for (let i = 0; i < basicCards.length; i += BATCH) {
        ...
        const results = await Promise.allSettled(
          batch.map(c =>
            fetch(`https://api.tcgdex.net/v2/en/cards/${setId}-${c.localId}`)
        ...
        results.forEach((result, idx) => {
          ...
          fullCards.push({
            ...
            name: basic.name,
            image: `https://assets.tcgdex.net/en/sv/${setId}/${basic.localId}/high.webp`,
