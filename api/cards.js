      ...
      try {
        ...
        if (scrydexId) {
          ...
          if (allCards.length > 0) {
            // For JP-phase sets with a real, registered TCGCSV group ID,
            // use the tested TCGCSV-primary + JP-fallback merge (Phase 1/2,
            // verified against real live data for me05: 120/120 cards, 0
            // duplicates, 0 needed from fallback since TCGCSV was already
            // complete). Sets without a registered group ID (the SET_TO_GROUP
            // check below) fall through to the exact original behavior,
            // completely unchanged -- this only ever activates for a set
            // that is BOTH phase='jp' AND has explicitly opted in here.
            ...
            if (bridgeGroupId) {
              try {
                ...
                // Shape the already-fetched Scrydex JP data to the format
                // mergeCards expects -- same translation.en preference as
                // the original code, just feeding the merge instead of
                // being used directly.
                ...
                cards = mergedCards.map(c => ({
                  ...
                  source: c.source, // 'tcgcsv' (confirmed) or 'jp-fallback' (estimate)
                  phase,
                }));
                ...
                // Falls through to the original Scrydex-only mapping below
              }
            }

            // Original Scrydex-only behavior -- unchanged, and still the
            // path taken for: all EN-phase sets, JP-phase sets with no
            // registered group ID, and as a safety fallback if the bridge
            // attempt above threw.
            if (!cards) {
